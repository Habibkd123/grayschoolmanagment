...
const typeConfig: Record<NewsType, { label: string; color: string; icon: React.ReactNode }> = {
  announcement: { label: "Announcement", color: "bg-[var(--primary)]/10 text-[var(--primary)] border-[var(--primary)]/20", icon: <Megaphone className="w-3.5 h-3.5" /> },
  circular: { label: "Circular", color: "bg-slate-100 dark:bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-300 border-slate-200 dark:border-slate-750", icon: <FileText className="w-3.5 h-3.5" /> },
  result: { label: "Result", color: "bg-[var(--primary)]/5 text-[var(--primary)]/70 border-[var(--primary)]/10", icon: <Award className="w-3.5 h-3.5" /> },
};
    ...
    <div className="space-y-6 max-w-4xl">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div className="w-10 h-10 rounded-xl bg-[var(--primary)]/10 border border-[var(--primary)]/20 flex items-center justify-center"><Newspaper className="w-5 h-5 text-[var(--primary)]" /></div>
          <div>
        ...
        <div className="flex items-center gap-3">
          <button onClick={addItem} className="flex items-center gap-1.5 px-4 py-2.5 rounded-lg bg-primary/10 border border-primary/20 text-primary hover:bg-primary/20 text-[13px] font-semibold transition-colors">
      ...
      {items.length === 0 ? (
        <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-12 text-center">
          ...
          <p className="text-slate-600 dark:text-slate-500 text-[12px] mt-1">Click "Add News" to create your first announcement.</p>
        </div>
      ) : (
              ...
              <div key={idx} className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-5 space-y-4">
                ...
                <div className="flex items-center gap-3 flex-wrap">
                  ...
                  <button onClick={() => updateItem(idx, "is_published", !item.is_published)}
                    className={`px-3 py-1.5 rounded-lg text-[11px] font-semibold border transition-colors ${item.is_published ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/20" : "bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-500 border-slate-200 dark:border-slate-750"}`}>
                    {item.is_published ? "● Published" : "○ Draft"}
                  </button>
                  <button onClick={() => removeItem(idx)} className="ml-auto p-1.5 text-slate-600 dark:text-slate-500 hover:text-rose-400 hover:bg-rose-500/10 rounded-lg transition-colors">
