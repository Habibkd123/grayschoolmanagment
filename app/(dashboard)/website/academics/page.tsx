    ...
    <div className="space-y-6 max-w-4xl">
      ...
      <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-6 space-y-5">
        ...
        {data.programs.length === 0 ? (
          <div className="text-center py-8 text-slate-600 dark:text-slate-500 text-[13px]">No programs defined. Click "Add Program" to get started.</div>
        ) : (
            ...
            {data.programs.map((program, idx) => (
              <div key={idx} className="p-4 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-800 space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={() => removeProgram(idx)} className="p-1.5 text-slate-600 dark:text-slate-500 hover:text-rose-400 hover:bg-rose-500/10 rounded-lg transition-colors">
      ...
      <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-6 space-y-5">
        ...
        {data.faculty.length === 0 ? (
          <div className="text-center py-8 text-slate-600 dark:text-slate-500 text-[13px]">No faculty members yet. Click "Add Faculty" to get started.</div>
        ) : (
          <div className="space-y-4">
            {data.faculty.map((member, idx) => (
              <div key={idx} className="p-4 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-800 space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={() => removeFaculty(idx)} className="p-1.5 text-slate-600 dark:text-slate-500 hover:text-rose-400 hover:bg-rose-500/10 rounded-lg transition-colors">
                    <Trash2 className="w-4 h-4" />
                  </button>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
