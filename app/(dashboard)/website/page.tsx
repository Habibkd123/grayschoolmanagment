...
import {
  ...
  AlertCircle, Loader2, ExternalLink, Eye, Info, Camera
} from "lucide-react";
...
const SECTIONS: Section[] = [
  {
    ...
    color: "from-[var(--primary)]/20 to-[var(--primary)]/10 border-[var(--primary)]/30",
  ...
  },
  {
    ...
    href: "/website/about",
    color: "from-[var(--primary)]/15 to-[var(--primary)]/8 border-[var(--primary)]/25",
      ...
      <div className={`relative rounded-2xl border bg-gradient-to-br ${section.color} p-6 transition-all duration-300 hover:scale-[1.02] hover:shadow-xl hover:shadow-black/20`}>
        ...
        <div className="absolute top-4 right-4">
          {isComplete ? (
            ...
            <span className="text-[11px] font-semibold text-slate-600 dark:text-slate-500 bg-slate-500/10 border border-slate-500/20 px-2.5 py-1 rounded-full">
              Empty
            </span>
          )}
        </div>
        ...
        {/* Progress bar */}
        <div className="mb-4">
          <div className="flex justify-between text-[11px] text-slate-600 dark:text-slate-500 mb-1.5">
        ...
        <div className="flex items-center justify-between">
          ...
          <ChevronRight className="w-4 h-4 text-slate-600 dark:text-slate-500 group-hover:text-primary group-hover:translate-x-1 transition-all" />
        </div>
      </div>
    </Link>
  );
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-6">
        ...
        <div className="flex justify-between text-[11px] text-slate-600 dark:text-slate-500 mt-2">
          <span>0%</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[
            ...
            <div key={item.tip} className="p-4 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
              ...
              <p className="text-slate-600 dark:text-slate-500 text-[11px] leading-relaxed">{item.detail}</p>
            </div>
          ))}
        </div>
      </div>
